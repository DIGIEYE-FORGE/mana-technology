...
export default function WidgetLabel({
  ...
  const avgData = useMemo(() => {
    /// {name: "Mining Dept", type: "area", data: Array(10)}
    return telemetries.map((item, index) => {
      const values = data?.[index]?.data.map((item) => item.y) || [];
      return {
        name: item.label || item.name,
        value: (values.reduce((a, b) => a + b, 0) / values.length).toFixed(4),
      };
    });
  }, [data, telemetries]);
      ...
      <div className="min-w-[18rem] p-4 py-10">
        {avgData.map((item) => (
          <div key={item.name} className="flex justify-between gap-2">
            ...
            <h4 className="text-sm font-semibold">{item.value}L</h4>
          </div>
        ))}
      </div>
    </div>
